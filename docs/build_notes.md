...
```mermaid
flowchart TD
  tcxZZJuliaCxxWrap-->|dll/so/dylib|TrussC_prebuilt_jll_packager.jl
  TrussC_prebuilt_jll_packager.jl-->TrussC_prebuilt_jll
  TrussC_prebuilt_jll-->TrussC.jl
```

1. Build dll/so/dylib by [tcxZZJuliaCxxWrap](https://github.com/funatsufumiya/tcxZZJuliaCxxWrap/) (In Win/Mac/Linux, this process is now manually done.)
...
The build process above is a little hard adn messy to test interactive binding changes of [tcxZZJuliaCxxWrap](https://github.com/funatsufumiya/tcxZZJuliaCxxWrap/).

So for development, we are using shorthand dll override process (for test only).

```mermaid
flowchart TD
  tcxZZJuliaCxxWrap-->|"dll/so/dylib (replace by hand)"|TrussC.jl
```
...
3. Update dll/so/dylib from [tcxZZJuliaCxxWrap](https://github.com/funatsufumiya/tcxZZJuliaCxxWrap/), then modify (override) the dll printed above by hand.
